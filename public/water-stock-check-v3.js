(() => {
  ...
  function isCopaCafeMenu() {
    ...
    const title = normalize(screen.querySelector('h1')?.textContent || '');
    return title === 'copa & cafe';
  }

  function findModuleGrid() {
    const screen = getActiveScreen();
    if (!screen || !isCopaCafeMenu()) return null;
    return screen.querySelector('.admin-grid.module-grid') || screen.querySelector('.module-grid');
  }
  ...
  function moveCardNearWaterCards(grid, card) {
    const references = Array.from(grid.querySelectorAll('.module-card')).filter((candidate) => candidate !== card);
    const waterReference = references.find((candidate) => {
      const title = normalize(candidate.querySelector('.module-card-title')?.textContent || '');
      return title === 'compras de agua';
    }) || references.find((candidate) => {
      const title = normalize(candidate.querySelector('.module-card-title')?.textContent || '');
      return title === 'estoque de agua';
    });
  ...
  function addConferenceCard() {
    ...
    let card = Array.from(document.querySelectorAll(CARD_SELECTOR)).find((candidate) => grid.contains(candidate));
    document.querySelectorAll(CARD_SELECTOR).forEach((candidate) => {
      if (candidate !== card && !candidate.closest('[aria-hidden="true"]')) candidate.remove();
    });
    ...
    moveCardNearWaterCards(grid, card);
  }
  ...
  function addStyles() {
    ...
    const oldStyle = document.querySelector('[data-water-stock-style="1"]');
    oldStyle?.remove();
    ...
    style.textContent = `
      ...
      .water-stock-header p{margin:0 0 2px;color:#ea580c;font-size:.72rem;font-weight:900;letter-spacing:.08em}
      ...
      .water-help{padding:14px;border:1px solid #fed7aa;border-radius:14px;background:#fff7ed}
      .water-help strong{display:block;color:#9a3412;font-size:1rem}
      ...
      .water-count-title span{display:inline-block;padding:4px 8px;border-radius:999px;background:#ffedd5;color:#9a3412;font-size:.7rem;font-weight:950}
      ...
      .water-count-card input:focus{border-color:#f97316;box-shadow:0 0 0 3px rgba(249,115,22,.12)}
      .water-count-card small{color:#64748b;font-weight:700;text-align:center}
      .water-save-area{display:grid;gap:8px}
      .water-save{min-height:54px;border:0;border-radius:12px;background:#f97316;color:#fff;font-size:1rem;font-weight:950;letter-spacing:.02em;cursor:pointer}
      ...
      .water-load-error{max-width:420px;text-align:center}.water-load-error p{color:#64748b}.water-load-error button{border:0;border-radius:10px;background:#f97316;color:#fff;padding:10px 16px;font-weight:900}
