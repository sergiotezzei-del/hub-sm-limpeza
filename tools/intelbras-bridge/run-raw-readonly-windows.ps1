...
Write-Host "Diagnostico BRUTO SOMENTE LEITURA em $panelHost`:$panelPort" -ForegroundColor Yellow
Write-Host "Device type: $deviceType" -ForegroundColor Yellow
Write-Host "Mostra somente metadados/cabecalhos recebidos. Nenhum payload sera exibido." -ForegroundColor Yellow
...
try {
    ...
    $env:INTELBRAS_DEVICE_TYPE = $deviceType
    $env:INTELBRAS_REMOTE_PASSWORD = $plainPassword

    node "$PSScriptRoot\raw-probe.mjs"
    exit $LASTEXITCODE
}
finally {
