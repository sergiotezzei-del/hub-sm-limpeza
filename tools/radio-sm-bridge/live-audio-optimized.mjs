...
server.listen(LIVE_PORT, "0.0.0.0", () => {
  console.log(`[radio-live] Ponte otimizada em http://${BIND_IP}:${LIVE_PORT}`);
  console.log("[radio-live] Perfil: estabilidade + baixa latencia; aguardando comando do HUB.");
});
...
function normalizeSource(remote) {
  const kind = ["system", "spotify", "edge", "chrome"].includes(remote?.source_kind) ? remote.source_kind : "system";
...
function validateLocalTools(source) {
  ...
  if (source.kind !== "system" && !fs.existsSync(PROCTAP_EXE)) throw new Error("Captura por aplicativo nao esta instalada.");
}
...
async function resolveRootProcessPid(processName) {
  ...
  if (!Number.isInteger(pid) || pid <= 0) throw new Error(`${processName} nao esta aberto neste notebook.`);
  return pid;
}

function runAndCollect(executable, args, timeoutMs) {
  return new Promise((resolve, reject) => {
    const child = spawn(executable, args, { cwd: BASE_DIR, windowsHide: true, stdio: ["ignore", "pipe", "pipe"] });
    let stdout = "";
    let stderr = "";
...
async function spawnCapture(source) {
  ...
  console.log(`[radio-live] Fonte: ${source.label} | PID raiz ${pid}`);
  return spawn(
    PROCTAP_EXE,
    ["--pid", String(pid), "--format", "int16", "--resample-quality", "fast", "--stdout"],
    { cwd: BASE_DIR, windowsHide: true, stdio: ["ignore", "pipe", "pipe"] },
  );
}
...
async function startLiveAudio(remote) {
  ...
  try {
    ...
    attachDiagnostics(state, capture, source.kind === "system" ? "capturador do Windows" : `capturador de ${source.label}`);
    ...
    console.log(`[radio-live] Ao vivo otimizado: ${source.label}.`);
  } catch (error) {
...
async function poll() {
  ...
  try {
    ...
    if (!desiredActive && liveState) await stopLiveAudio();
  } catch (error) {
...
async function shutdown() {
  ...
  console.log("\n[radio-live] Encerrando ponte otimizada...");
