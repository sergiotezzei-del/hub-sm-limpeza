...
declare
  ...
  v_source_service_request_status text;
begin
  if tg_op = 'INSERT' then
    if new.source_service_request_id is not null then
      select
        ...
        sr.status
      into
        v_source_service_request_protocol,
        v_source_service_request_status
    ...
    if new.source_service_request_id is not null then
      insert into public.service_request_events (
        ...
        new.source_service_request_id,
        'anotacao',
        v_source_service_request_status,
        v_source_service_request_status,
        new.last_actor_name,
        format('Chamado adicionado aos Afazeres. Tarefa criada: %s. ID: %s.', new.title, new.id)
      );
    end if;
