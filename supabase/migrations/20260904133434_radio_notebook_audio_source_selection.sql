alter table public.radio_notebook_audio_state
  ...
  add column if not exists source_process_name text,
...
alter table public.radio_notebook_audio_state
  ...
  check (source_kind in ('system','spotify','edge','chrome'));

update public.radio_notebook_audio_state
   set source_kind = coalesce(nullif(source_kind,''), 'system'),
       source_process_name = case coalesce(nullif(source_kind,''), 'system')
         when 'spotify' then 'Spotify.exe'
         when 'edge' then 'msedge.exe'
         when 'chrome' then 'chrome.exe'
         else null
       end,
       source_label = case coalesce(nullif(source_kind,''), 'system')
...
drop function if exists public.radio_start_notebook_audio();
...
begin
  ...
  if v_kind not in ('system','spotify','edge','chrome') then
...
drop function if exists public.radio_bridge_get_notebook_audio(text);

create function public.radio_bridge_get_notebook_audio(p_token text)
returns table(
  active boolean,
  status text,
  source_kind text,
...
begin
  ...
  return query
  select s.active, s.status, s.source_kind, s.source_process_name, s.source_label
